...
import CreatePdfEditor from './CreatePdfEditor';
import CompressPdfEditor from './CompressPdfEditor';
import ExtractPdfImagesEditor from './ExtractPdfImagesEditor';

interface PdfStudioProps {
  initialMode: EditMode;
}

const PdfStudio: React.FC<PdfStudioProps> = ({ initialMode }) => {
    ...
    const renderActiveTool = () => {
        switch (activeTool) {
            case EditMode.CreatePDF:
                return <CreatePdfEditor />;
            case EditMode.CompressPDF:
                return <CompressPdfEditor />;
            case EditMode.ExtractPdfImages:
                return <ExtractPdfImagesEditor />;
            default:
                return <CreatePdfEditor />;
        }
    };
        ...
        <div className="flex flex-col h-full">
            <div className="flex-shrink-0 mb-4">
                <div className="flex items-center gap-2 p-2 bg-base-100 rounded-lg overflow-x-auto">
                    {MENU_STRUCTURE.PDF.tools.map(mode => (
                        <button 
                            key={mode} 
