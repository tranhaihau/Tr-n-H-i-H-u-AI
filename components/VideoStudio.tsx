...
const VideoStudio: React.FC<VideoStudioProps> = ({ initialMode }) => {
    ...
    const renderActiveTool = () => {
        switch (activeTool) {
            case EditMode.GenerateVideo:
                return <VideoGenerator />;
            case EditMode.GenerateVideoFromScript:
                return <VideoScriptGenerator />;
            case EditMode.RemoveWatermarkVideo:
        ...
        <div className="flex flex-col h-full">
            <div className="flex-shrink-0 mb-4">
                <div className="flex items-center gap-2 p-2 bg-base-100 rounded-lg overflow-x-auto">
                    {MENU_STRUCTURE.VIDEO.tools.map(mode => (
                        <button 
                            key={mode} 
